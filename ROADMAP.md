...
### Draft
- [?] Entropy gate is still a blunt instrument for structurally-anchored rules: after #T113, ~18% of random 20-char keys and ~32% of random 32-char tokens are still missed (5000-sample measurement). For a rule like `aws-access-token` — fixed `AKIA` prefix plus an exact `[A-Z2-7]{16}` shape — the structure alone establishes it is a key, so the entropy gate is nearly pure loss; a false positive would require someone to write an AKIA-prefixed 20-char uppercase base32 string by hand. Proposal: set `entropy: false` on rules with a fixed-prefix or exact-length anchor, keeping the gate only for keyword-proximity rules. Needs a per-rule audit of all 108 anchored rules with before/after false-positive counts over the repo, so it is not a blind sweep (model: opus) #T116
- [?] 222 of 233 rules have zero test cases (`test-rules` reports "11 passed, 222 warned"), so a rule can silently stop matching with no signal. Add representative test cases for at least every CRITICAL-severity rule (depends: #T113) PARTIAL: #T114 covers the 21 restored rules via tests/scan-rules-restored.test.ts. `test-rules` itself still reports 11 passed / 222 warned, because inline fixtures in scan-rules.js trip GitHub push protection — so closing this task means either runtime-built cases in the node suite, or a fixture-generation step that keeps literals out of the committed data file. Remaining scope is the pre-existing CRITICAL-severity rules that still have none #T115
