...
### Review
### Done
- [x] Completed tool result counted toward context pressure. First of three findings in a review of the #T126 commit, confirmed before anything changed. `PostToolUse` fires after the tool produced its result but before the model's next request, so the newest assistant `usage` record describes the request that *asked for* the tool — the result now sitting in context is invisible to the measurement until one more turn has gone by. Ordinarily that lag is harmless, because the nudge line sits 15 points below the compaction line and the gap absorbs a turn. It is not harmless when a single result crosses the whole gap: a large `Read` or a wide `Grep` can add ~30k tokens in one step, taking the session from below the nudge line to at or past the compaction line without ever being measured in between, and the once-per-cycle nudge is then never owed at a moment it could still be delivered — the cycle is lost outright, which is the same failure mode #T125 fixed by a different route. `compact-suggest.sh` now adds `PENDING_TOKENS=${#INPUT}/4` to the measured count, in the token branch only: the byte-proxy branch already scales with payload size, so adding it there would double-count. Recorded in the hook comment and here as **explicitly a proxy, not a measurement** — 4 bytes/token is the English-prose ratio and JSON and source code both run denser, so the correction under-estimates. That direction is deliberate and is the safe one: it narrows the gap between the measured number and the real one without closing it, so the hook can still nudge late but cannot nudge on a number it invented. #T127
- [x] Transcript record type parsed structurally rather than textually. Second finding of the same review, confirmed before anything changed. #T122's guard required `"type":"assistant"` somewhere on the line, which is a whole-line substring test against a record that nests arbitrary JSON: a `toolUseResult` carrying an assistant-shaped object satisfies it, and the scan then measures a tool payload as the session's context. Probe data settled how exposed this was — `toolUseResult` is a structured object in 687 of 2958 records (plus 65 arrays), so unescaped nested JSON is the norm rather than an edge case, and the nested content is not escaped. Escaped occurrences were never the risk: `\"type\"` carries a backslash before the quote, so the regex never matched it. The same probe closed off the cheap fix — `message` is serialized *ahead* of the top-level `type` in all 1185 assistant records (mean offset 2194 bytes, max 33108), so no prefix-scan shortcut is sound — and made the strict version free, since all 1185 `"usage":` keys sit at exactly depth 2. The awk now reduces each record to a brace/bracket skeleton, substituting sentinels for the three keys it reasons about *while the quotes are still intact*, then stripping escapes and string bodies; `type:assistant` and `isSidechain` count only at depth 1 and only a depth-2 `usage` key is measured, with a record whose only `usage` lives deeper skipped outright rather than measured on the payload that happens to be there. Prefiltered on `index($0,"\"usage\":")` so the skeleton work stays off the common path. Recorded honestly, as #T122 recorded its own: 0 of 1182 raw-regex hits had a non-assistant top-level type and 0 unescaped occurrences sat outside depth 1 in the transcript this was measured against — structural hardening against a silent failure, not a fix for an observed one. #T128
- [x] Handoff ownership claimed ahead of the write it describes. Third finding of the same review, confirmed before anything changed. The claim in `compact-suggest.sh` ran on `PostToolUse`, so between the `Write` creating `handoff-*.yaml` and the hook recording who wrote it there is a window where the file is discoverable and unattributed — a concurrent session compacting inside it takes the fresh handoff off the fallback glob and forwards a `compact_instruction` written for someone else's task, the #T120 leak reached by timing rather than by a missing record. Narrow, but it widens with hook latency and it is the failure this chain has now fixed three times. Fixed by inverting the order rather than by adding synchronization: `compact-suggest.sh` is now registered on `PreToolUse` for the write tools as well, where it records the claim and exits — so the claim is on record before the artifact can be discovered — with the `PostToolUse` pass kept as a backstop for a write this session did not see the start of. Both passes collapse against the last line, so claiming twice costs nothing. Exiting 0 and printing nothing on that path is load-bearing: `PreToolUse` is the one event where a hook can deny the tool call, and an advisory hook must never do that. Rejected the alternative of encoding ownership in the published artifact for the reason recorded twice before — the model authoring a handoff has no reliable way to learn its own session id. Also added the suite's first **hook-registration** assertions, reading the real `.claude/settings.json`: every other assertion invokes hooks directly, and the claim block reads `file_path` out of `tool_input` and works on a `PreToolUse` payload whether or not it is registered for that event, so the entire pre-claim could have been dead in production with the suite green. Seven of the eighteen new assertions verified to fail against the pre-fix hooks and pre-fix settings — three for #T128, one for #T127, three for this; the rest pin properties the fixes must not break. Tests 140 -> 158 assertions #T129
