...
build_template_checkout() {
    ...
    # templates/ is the CONTENT SEED TIER new-project.sh copies from
    # (CONTENT_FILES). Without it, bootstrap/adopt from this checkout hard-fails
    # with "template file missing". Any tool that assembles a Project OS
    # checkout must include it -- see templates/README.md.
    cp -r "$REPO_ROOT/templates" "$dest/templates"
    mkdir -p "$dest/docs/knowledge" "$dest/docs/proposals"
    local f
    # Framework REFERENCE docs shipped verbatim (not seeds): shipped files point
    # at these, so a checkout without them produces dangling refs in every
...
# --- pre-push hook survives a FIRST push (no origin/BRANCH) -------------------
# A new project has no upstream, so `scan-diff "origin/$BRANCH"` used to abort
# with "fatal: ambiguous argument" — the hook failed on a GIT ERROR, not a
# finding, and told the user to bypass with --no-verify, disabling secret
# scanning exactly when a repo first becomes remote.
if [ -f "$PROJ/.git/hooks/pre-push" ]; then
    pass "setup.sh installed the git pre-push hook"
    set +e
    PREPUSH_OUT="$( (cd "$PROJ" && bash .git/hooks/pre-push </dev/null 2>&1) )"
    PREPUSH_EC=$?
    set -e
    if [ "$PREPUSH_EC" -eq 0 ]; then
        pass "pre-push hook exits 0 on a first push (no origin/BRANCH)"
    else
        fail "pre-push hook failed on first push (exit $PREPUSH_EC): $PREPUSH_OUT"
    fi
    assert_not_contains "$PREPUSH_OUT" "ambiguous argument" "pre-push does not hit git's ambiguous-argument error"
    assert_contains "$PREPUSH_OUT" "scanning all tracked files" "pre-push falls back to a full tracked-file scan"

    # The fallback must still BLOCK a real secret — a hook that always exits 0
    # is worse than the original bug.
    #
    # Payload choice matters: the canonical AWS example key AKIAIOSFODNN7EXAMPLE
    # is (correctly) in .claude/security/allowlist.json's stopwords, and any
    ...
    LEAK_EC=$?
    set -e
    if [ "$LEAK_EC" -ne 0 ]; then
        pass "pre-push first-push fallback still blocks a planted secret"
    else
        fail "pre-push fallback did NOT block a planted AWS key — scanning is inert"
    fi
    git -C "$PROJ" rm -q --cached leak-probe.js >/dev/null 2>&1
    rm -f "$PROJ/leak-probe.js"
else
    fail "git pre-push hook not installed by setup.sh"
fi

# --- Secret scanner actually loads its rules on this Node ---------------------
# scan-rules.js used inline regex modifiers requiring Node >= 23 while
# package.json declares >= 22.18; on Node 22 the module was a syntax error, so
# hook install died and a project silently had NO secret scanning at all.
set +e
RULES_OUT="$( (cd "$PROJ" && node scripts/security-scanner.ts test-rules --quiet 2>&1) )"
RULES_EC=$?
set -e
if [ "$RULES_EC" -eq 0 ]; then
    pass "security scanner rules load and validate in the new project"
else
    fail "security scanner rules failed to load (exit $RULES_EC): $(printf '%s' "$RULES_OUT" | head -2)"
fi
assert_not_contains "$RULES_OUT" "Invalid regular expression" "no rule uses regex syntax unsupported on this Node"
...
# Scope to the "files" block: the manifest also carries a "seed_hashes" block
# keyed on the same paths, so an unscoped grep matches twice and yields an
# ambiguous value.
MANIFEST_LINE="$(sed -n '/"files"[[:space:]]*:[[:space:]]*{/,$p' "$S9/.claude/manifest.json" 2>/dev/null | grep '"docs/knowledge/decisions.md":' || true)"
