...
- `decide_scrubSubprocessNonZeroButRescanClean_proceeds`: the guard deliberately ignores the scrub subprocess's own exit status and trusts only the positive re-scan, which is itself fail-closed (a leftover secret fails that re-scan). So a non-zero scrub status ALONE does not decline: scrub stub returns status 1, scan stub returns 0; asserts the call proceeds to the Jev backend (`backend === "jev"`, `declined === undefined`, fetch call count `1`). A failed positive re-scan is what declines `"scrub-failed"` — see `decide_guardRefused_declinesWithGuardReason`.
