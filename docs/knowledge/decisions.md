...
3. **The requirement rides PostToolUse `additionalContext`.** `compact-suggest.sh` is the only place in the chain that can put text into Claude's context. When transcript growth since the last compaction passes `PROJECT_OS_COMPACT_NUDGE_BYTES`, it injects an instruction to run `/tools:handoff` with a `compact_instruction` — once per compaction cycle, reset by `pre-compact.sh`.
4. **The drafted compact message rides PreCompact stdout.** The runtime collects the stdout of every successful PreCompact hook into `newCustomInstructions` and merges it into the compaction's custom instructions. `pre-compact.sh` prints the newest handoff's `compact_instruction` block scalar (freshness window 30 min), so the field the model wrote becomes guidance the summarizer actually receives. Consequence, documented in the script header: stdout must be plain text — a JSON envelope would be forwarded verbatim as instruction text.
...
- **Count tool calls as the pressure proxy** (the previous `compact-suggest.sh` behaviour) — rejected: uncorrelated with context size; one `Read` of a large file outweighs fifty `Bash` calls. Transcript bytes since the last compaction is a coarser-grained but monotonic stand-in. Hooks receive no token count; there is no exact signal available.

**Rationale**: A hook cannot write a handoff — only the model holds the decisions and rationale worth preserving, and `pre-compact.sh` can see nothing but ROADMAP markers and `git status`. So the honest design is: ask the model early enough that it still has the context to answer, then carry its answer forward to the summarizer. Both halves use channels verified against the shipped runtime rather than inferred from documentation. The residual weakness is stated rather than hidden — `PROJECT_OS_COMPACT_NUDGE_BYTES` is an uncalibrated proxy for a quantity hooks are not given, and it will need tuning against real sessions.
